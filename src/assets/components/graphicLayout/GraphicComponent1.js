...
export default function GraphicComponent1(props) {	
	useEffect(() => {	
		const CTX = document.querySelector('#daily').getContext('2d');
		preparation(CTX, props.data.length);
		canvasBuild(CTX, {xMax: props.data.length, yMax: 70}, 'red', props.data.map(item => item.temperature.max));
		canvasBuild(CTX, {xMax: props.data.length, yMax: 70}, 'blue', props.data.map(item => item.temperature.min));
	});
	...
	return (		
		<div className="canvas__wrapper">
			<canvas id="daily" className="canvas__container" width='200' height='100'></canvas>
